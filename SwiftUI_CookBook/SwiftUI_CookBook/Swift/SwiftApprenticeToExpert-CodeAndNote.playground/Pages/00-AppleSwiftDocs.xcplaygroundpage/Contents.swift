//: # Table of Contents
/*:
 - Swift Language
 - SwiftUI
 - WWDC
 - Additional Resources & Blogs
 - Therd Party Libraries
 - Apple Developer Sites
 - SwiftUI Tutorials
 - Programable APIs
 */

/*:
 ## Swift Language
 - [swift.org](https://www.swift.org/)
 - [swift-evolution](https://github.com/apple/swift-evolution)
 - [Swift Evolution Proposals](https://github.com/apple/swift-evolution/tree/main/proposals)
 - [Swift Forums](https://forums.swift.org/)
 - [Apple Developer Documentation](https://developer.apple.com/documentation/)
 */

/*:
 ## SwiftUI
 - [Apple Developer - SwiftUI Tutorials](https://developer.apple.com/tutorials/swiftui)
 - [Big Mountain Studio](https://www.bigmountainstudio.com/)
    - SwiftUI Views Mastery
    - Working with Data in SwiftUI
    - Core Data Mastery in SwiftUI
    - Combine Mastery in SwiftUI
    - [SwiftUI Accelerator Snippets Vault & Mini-Course](https://www.bigmountainstudio.com/view/courses/snippets)
    - [SwiftUI in UIKit Weekend Kickstarter](https://www.bigmountainstudio.com/view/courses/kickstarter)
    - [Cdong With AI not available now](https://www.bigmountainstudio.com/ai-coder)
 */

/*:
 ## WWDC
 - [WWDC](https://developer.apple.com/wwdc23/)
 - [All Apple Technologies Docs](https://developer.apple.com/documentation/technologies)
 - [UIKit Documentation](https://developer.apple.com/documentation/uikit)
 - [SF Symbols](https://developer.apple.com/sf-symbols/)
 - [TipKit](https://developer.apple.com/documentation/tipkit)
 - [String Catalog](https://developer.apple.com/documentation/Xcode/localizing-and-varying-text-with-a-string-catalog)
 - Swift Data
     - [Documentation](https://developer.apple.com/documentation/SwiftData)
     - [WWDC 2023 Video](https://developer.apple.com/videos/play/wwdc2023/10196/)
 - [DocC](https://developer.apple.com/documentation/docc)
 */

//: ## Additional Resources & Blogs
/*:
 - [Kodeco](https://www.kodeco.com/home)
 - [Hacking with Swift](https://www.hackingwithswift.com/)
 - [academy-realm-io](https://academy.realm.io/section/apple)
 - [Point-Free](https://www.pointfree.co/) - TCA and Quality Posts
 - [SwiftDoc](https://swiftdoc.org/)
 - [Swift by Sundell](https://www.swiftbysundell.com/)
 - [Fatbobman's Swift Notebook](https://fatbobman.com/)
 - [NSHipster](https://nshipster.com/)
 - [NSHipster 中文版](https://nshipster.cn/)
 - [Antoine van der Lee's Blog](https://www.avanderlee.com/)
 - [Cocoa Conferences](https://cocoaconferences.com/)
 - [Try Combine](https://trycombine.com/)
 - [Zhiying's Space](https://zhiying.space/)
 - [iOS Dev Directory](https://iosdevdirectory.com/#zh)
 - [Jorge Lascorbe's Blog](https://lascorbe.com/)
 - [iOS Dev Weekly](https://iosdevweekly.com/)
 - [iOS Goodies](https://ios-goodies.com/)
 - [WWDC Notes](https://www.wwdcnotes.com/)
 - [Five Stars Blog](https://www.fivestars.blog/)
 - [SwiftUI Recipes](https://swiftuirecipes.com/)
 - [SwiftyPlace Blog](https://www.swiftyplace.com/blog)
 - [Sarunw](https://sarunw.com/)

 Commertials:
 - [Kavsoft](https://kavsoft.dev/) 5$ / month
 - [DevTechie](https://www.devtechie.com/)
 - [AppCoda](https://www.appcoda.com/)
 - [Design+Code](https://designcode.io/)
 */

/*:
 ## Therd Party Libraries
 - [Realm](https://realm.io/realm-swift/)
 - [Vapor](https://docs.vapor.codes/)
 */

/*:
 ## Apple Developer Sites
 - [Apple ID Sign-In](https://appleid.apple.com/sign-in)
 - [Apple Developer Account](https://developer.apple.com/account)
 - [Apple Developer](https://developer.apple.com/)

 */

/*:
 ## SwiftUI Tutorials:
 - [Apple SwiftUI Tutorials](https://developer.apple.com/tutorials/swiftui)
 - [Apple Developer Sample Apps Tutorials](https://developer.apple.com/tutorials/sample-apps)
 */

/*:
 ## Programable APIs
 - [Yelp Developers](https://www.yelp.com/developers?country=US)
 - [The Movie Database (TMDb) Developer Documentation](https://developer.themoviedb.org/docs)
 - [Unsplash Developers](https://unsplash.com/developers)
 */
